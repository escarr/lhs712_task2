...
num_tweets_min <- c(5, 3) #tune threshold number of tweets a word must appear in to see impact on F1
...
#replace brand name drugs with the word "drug" to reduce the number of features
count_replaced <- 0 #count number of words replaced by "drug" for report
for (i in 1:nrow(training_data_tokenized_no_stops)) {
	if (training_data_tokenized_no_stops[i, "word"] %in% brand_names$word) {
		training_data_tokenized_no_stops[i, "word"] <- "drug" #replace brand name with drug
		count_replaced <- count_replaced + 1
	}
}

#print stats related to number of words replaced by 'drug'
print(paste(((count_replaced/nrow(training_data_tokenized_no_stops))*100), 
			" % of words were brand names and replaced by 'drug' based on RxTerms (March, 2020)", sep = "")) #proportion of words replaced by 'drug'
